...
Camera::Camera(glm::vec3 startPosition) : spotlightOn(false), position(startPosition), front(0.0f, 0.0f, -1.0f), up(0.0f, 1.0f, 0.0f), right(1.0f, 0.0f, 0.0f)
                                        ,yaw(-90.0f), pitch(-20.0f), velocity(0.0f), targetVelocity(0.0f), moveSpeed(5.0f), turnSpeed(60.0f), fKeyWasPressed(false){
    updateVectors();
}
...
glm::vec3 Camera::getSpotlightPosition() const {
    return position;
}

glm::vec3 Camera::getSpotlightDirection() const {
    return front;
}

float Camera::getCutoffAngle() const {
